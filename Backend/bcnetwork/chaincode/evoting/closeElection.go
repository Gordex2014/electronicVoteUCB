...
func closeElection(stub shim.ChaincodeStubInterface) peer.Response {
	var err error
	configJSON := Config{}
	...
	if configJSON.ElectionOpen == false {
		return errorResponse("La elección ya se encuentra cerrada", 400)
	}

	// Abrir la mesa
	configJSON.ElectionOpen = false
	...
	if err != nil {
		return errorResponse("Error en el interno al cerrar la eleccón datos, revise la configuracion", 500)
	}

	return successResponse("Elección cerrada correctamente")
